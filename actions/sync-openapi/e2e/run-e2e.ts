...
async function githubApi(
  ...
  // biome-ignore lint/suspicious/noExplicitAny: GitHub API returns varied shapes
): Promise<any> {
...
async function getFileSha(path: string): Promise<string> {
  const data = await githubApi(`/repos/${OWNER}/${REPO}/contents/${path}?ref=main`);
  return data.sha;
}
 ...
 */
async function resetFernSpec(branch = "main"): Promise<void> {
  const content = Buffer.from("{}").toString("base64");
  const data = await githubApi(`/repos/${OWNER}/${REPO}/contents/${FERN_SPEC_PATH}?ref=${branch}`);
...
async function deleteBranch(): Promise<void> {
  try {
    ...
    // biome-ignore lint/suspicious/noExplicitAny: need to check error message
  } catch (e: any) {
...
async function waitForWorkflowRun(afterDate: Date, timeoutMs = 300000): Promise<WorkflowRun> {
  ...
  while (Date.now() < deadline) {
    ...
    // biome-ignore lint/suspicious/noExplicitAny: workflow run items have dynamic shape
    const run = runs.find((r: any) => new Date(r.created_at) > afterDate);

    if (run) {
      if (run.status === "completed") {
        console.log(`  Workflow run ${run.id} completed: ${run.conclusion}`);
        return run;
      }
...
async function getOpenPRs(): Promise<GitHubPR[]> {
  return githubApi(`/repos/${OWNER}/${REPO}/pulls?head=${OWNER}:${BRANCH}&state=open`);
}
...
async function pushConflictingCommit(branchSha: string): Promise<void> {
  ...
  const commit = await githubApi(`/repos/${OWNER}/${REPO}/git/commits/${branchSha}`);
...
async function testHappyPath(): Promise<void> {
  ...
  if (run1.conclusion !== "success") {
    throw new Error(`Expected first run to succeed, got: ${run1.conclusion} (${run1.html_url})`);
  }

  const prs1 = await getOpenPRs();
  if (prs1.length !== 1) {
    throw new Error(`Expected 1 open PR after first run, got ${prs1.length}`);
  }
  ...
  if (run2.conclusion !== "success") {
    throw new Error(`Expected second run to succeed, got: ${run2.conclusion} (${run2.html_url})`);
  }

  const prs2 = await getOpenPRs();
  if (prs2.length !== 1) {
    throw new Error(`Expected still 1 open PR after second run, got ${prs2.length}`);
  }

  if (prs2[0].number !== prNumber) {
    throw new Error(`Expected same PR #${prNumber}, got PR #${prs2[0].number}`);
  }

  const commits2 = await getPRCommitCount(prNumber);
  if (commits2 <= commits1) {
    throw new Error(`Expected more commits after second run (had ${commits1}, now ${commits2})`);
  }
...
async function testConflictPath(): Promise<void> {
  ...
  const branchData = await githubApi(`/repos/${OWNER}/${REPO}/git/refs/heads/${BRANCH}`);
  ...
  const updatedBranch = await githubApi(`/repos/${OWNER}/${REPO}/git/refs/heads/${BRANCH}`);
  ...
  if (run.conclusion === "failure") {
    ...
    console.log("  Workflow succeeded - rebase may have resolved the conflict, or timing was off.");
  ...
  if (conflictComment) {
    ...
    console.log(`  before the PR comment step. Check logs: ${run.html_url}`);
    ...
    console.log("  The conflict handling logic is thoroughly covered by unit tests.");
