...
async function run(inputs: ActionInputs): Promise<void> {
  ...
  // 1. Detect generator groups eligible for preview via the CLI.
  //    `fern automations list preview --json` is the single source of truth.
  const groups = await detectPreviewGroups({ fernToken: inputs.fernToken });
  if (groups.length === 0) {
